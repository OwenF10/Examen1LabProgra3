...
int main(int argc, char* args[])
{
    ...
    int frames = 0;
    int cant = 0;
    Mix_Chunk *gEat = Mix_LoadWAV("audio/awesome.wav");
    ...
    Mix_Music *Music= Mix_LoadMUS( "audio/unity.wav" );
    ...
    while(true)
    {
        ...
        while(SDL_PollEvent(&Event))
        {
            ...
            if(Event.type == SDL_KEYDOWN)
            {
                ...
                {
                    saveScreen = true;
                    ofstream out("partidas.txt");
                    out << rect_character.x << " " << rect_character.y << " " << velocity << " " << dir;
                    out.close();
                    game = false;
                }
                if(Event.key.keysym.sym == SDLK_l && startScreen)
                {
                    ...
                    in >> rect_character.x >> rect_character.y >> velocity >> dir;
                    in.close();
                    startScreen = false;
                    game = true;
